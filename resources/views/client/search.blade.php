                ...
                <form class="row" method="GET">
                        ...
                        <div class="form-group">
                            <select class="form-control form-select" name="brand">
                                ...
                                @foreach ($brands as $item)
                                    <option value="{{ $item->id }}" {{ $item->id == $brand ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-sm-4">
                        <div class="form-group" style="display: flex; align-items: center;">
                            <input type="text" class="form-control price-filter" value="{{ $minPrice }}"
                                placeholder="Giá từ" name="min_price">
                            <span style="border-top: 1px; width: 50px;"></span>
                            <input type="text" class="form-control price-filter" value="{{ $maxPrice }}"
                                placeholder="Giá đến" name="max_price">
                        </div>
                    </div>
                    <div class="col-sm-2">
                        <div class="form-group">
                        ...
                        <div class="row">
                            @foreach ($products as $product)
                                <div class="col-md-3 col-sm-6">
                                    <div class="products">
                                        <div class="thumbnail">
                                            <a href="{{ route('user.products_detail', $product->id) }}"><img
