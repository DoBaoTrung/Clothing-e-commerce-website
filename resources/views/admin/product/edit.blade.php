...
@section('content')
    <section section class="content">
        <div class="container-fluid">
            <div class="row">
                ...
                <form class="row" action="{{ route('admin.products_update', $product->id) }}" method="POST"
                    id="form__js" enctype="multipart/form-data">
                    @csrf
                                ...
                                <div class="col-8">
                                    <div class="card-body row">
                                        ...
                                        <x-admin-input-prepend label="Giá Nhập" col="col-6" width="auto">
                                            <input id="price_import" type="number" min="1" name="price_import"
                                                value="{{ $product->price_import }}" class="form-control">
                                        </x-admin-input-prepend>
                                        <x-admin-input-prepend label="Giá Bán" col="col-6" width="auto">
                                            <input id="price_sell" type="number" name="price_sell"
                                                value="{{ $product->price_sell }}" class="form-control">
                                        </x-admin-input-prepend>
                                        <x-admin-input-prepend label="Thương Hiệu" width="auto" col="col-6">
                                            <select class="form-control" name="brand_id" id="brand">
                                                @foreach ($brands as $brand)
                                                    <option value="{{ $brand->id }}"
                                                        @if ($brand->id == $product->brand_id) @selected(true) @endif>
                                                        {{ $brand->name }}</option>
                                                @endforeach
                                            </select>
                                        </x-admin-input-prepend>
                                            ...
                                            <select class="form-control" name="parent_id" id="parent_id">
                                                @foreach ($categoriesParent as $categoryParent)
                                                    <option value="{{ $categoryParent->id }}"
                                                        @if ($categoryParent->id == $product->category->parent_id) @selected(true) @endif>
                                                        {{ $categoryParent->name }}</option>
                                                @endforeach
                                            </select>
                                        </x-admin-input-prepend>
                                        <x-admin-input-prepend label="Danh Mục" width="auto">
                                            <select class="form-control" value="{{ $product->category_id }}"
                                ...
                                <div class="col-4">
                                    <div class="card-body">
                                        <div class="container">
                                            <div class="preview">
                                                <img id="img-preview"
                                                    src="{{ asset("asset/client/images/products/small/$product->img") }}" />
    ...
    <script></script>
    @vite(['resources/admin/js/user-create.js', 'resources/admin/js/edit-product.js', 'resources/admin/css/product.css', 'resources/admin/css/form-edit.css', 'resources/common/js/form.js'])
@endsection
